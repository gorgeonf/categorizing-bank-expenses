...
def generate_summary_balance_graph(statement: DataFrame, type: str):
...
def generate_period_summary_balance_bar_graph(sliced_statements: list):
    ...
    bars_income = ax.bar(x - width / 2, income_values, width, color=colors[0], label=labels[0])
    bars_expenses = ax.bar(x + width / 2, expenses_values, width, color=colors[1], label=labels[1])
